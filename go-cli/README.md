...
boru-driven command-line client **and** interactive REPL for the NobelPrize
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/nobel-prize-cli)
make build

# 2. See usage (words, entities, env vars)
./nobel-prize-cli --help

# 3. Provide credentials once, via the environment
export NOBEL_PRIZE_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./nobel-prize-cli list laureate
./nobel-prize-cli list prize

# 5. Override the API base URL for a single call
NOBEL_PRIZE_BASE=https://api.example.com ./nobel-prize-cli list laureate

# 6. No arguments -> interactive REPL
./nobel-prize-cli
nobel-prize> list laureate
nobel-prize> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/nobel-prize-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/nobel-prize-cli list laureate
   ```
...
Run with no arguments to open a REPL (prompt `nobel-prize>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `laureate`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/nobel-prize-cli`. |
...
(`./nobel-prize-cli <expr>`) and interactively (the REPL), and expressions compose the
