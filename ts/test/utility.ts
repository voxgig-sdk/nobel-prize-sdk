...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.NOBELPRIZE_TEST_LIVE ||
    'TRUE' === process.env.NOBELPRIZE_TEST_OVERRIDE
  ) {
  ...
  m.NOBELPRIZE_TEST_EXPLAIN = process.env.NOBELPRIZE_TEST_EXPLAIN || m.NOBELPRIZE_TEST_EXPLAIN
