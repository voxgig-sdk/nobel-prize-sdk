...
describe('LaureateEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    let laureate_ref01_data = Object.values(setup.data.existing.laureate)[0] as any

    // LIST
    const laureate_ref01_ent = client.Laureate()
    const laureate_ref01_match: any = {}

    const laureate_ref01_list = (await laureate_ref01_ent.list(laureate_ref01_match)).map((e: any) => e.data())
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/laureate/LaureateTestData.json')
