...
//     (\`new NobelPrizeSDK(...)\` / \`NobelPrizeSDK.test(...)\` become
//     \`NobelPrizeSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"laureate":{"test01":{"id":"test01"}},"prize":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new NobelPrizeSDK(...)\` and any
// \`NobelPrizeSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+NobelPrizeSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/NobelPrizeSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
