...
class FtCtrl extends NobelPrizeControl
{
...
class FtClient
{
    ...
    private NobelPrizeUtility $utility;

    public function __construct(NobelPrizeUtility $utility, array $options)
    {
    ...
    public function get_utility(): NobelPrizeUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (NobelPrizeContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public NobelPrizeContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (NobelPrizeContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = NobelPrizeConfig::make_feature($name);
    ...
    private static function buildUrl(NobelPrizeSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof NobelPrizeError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new NobelPrizeResponse($fetched);
        }
    ...
    private function fail(NobelPrizeContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (NobelPrizeContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
