...
class LaureateEntity
  ...
  def make
    opts = @_entopts.dup
    LaureateEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = NobelPrizeHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Laureate, Hash] the current Laureate data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = NobelPrizeHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Laureate fields)
  def match_get
  ...
  #
  # @param reqmatch [LaureateListMatch, Hash, nil] match filter (any subset of
  #   Laureate fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Laureate>, Array] the matching Laureate items; raises NobelPrizeError on failure
