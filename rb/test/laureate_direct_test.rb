...
class LaureateDirectTest < Minitest::Test
  def test_direct_list_laureate
    setup = laureate_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-laureate", setup[:live] ? "live" : "unit")
    if _should_skip
...
def laureate_direct_setup(mockres)
