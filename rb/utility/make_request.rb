...
module NobelPrizeUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = NobelPrizeResponse.new({})
    ...
    if fetch_err
      ...
      response = NobelPrizeResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = NobelPrizeResponse.new(fetched)
    else
