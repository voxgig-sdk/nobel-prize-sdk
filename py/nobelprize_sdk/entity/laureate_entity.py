...
from nobelprize_sdk.nobelprize_types import (
    Laureate,
    LaureateListMatch,
)
...
class LaureateEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return LaureateEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Laureate]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Laureate().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
