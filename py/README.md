...
The Python SDK for the NobelPrize API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Laureate(None).list(None, None)
if err:
    raise Exception(err)
...
result, err = client.NobelPrize(None).load(
    {"id": "test01"}, None
)
