...
class NobelPrizeTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            NobelPrizeTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = NobelPrizeTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = NobelPrizeTestRunner.getenv("NOBEL_PRIZE_TEST_LIVE")
        override = NobelPrizeTestRunner.getenv("NOBEL_PRIZE_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = NobelPrizeTestRunner.getenv(key)
        ...
        explain = NobelPrizeTestRunner.getenv("NOBEL_PRIZE_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if NobelPrizeTestRunner._test_control is not None:
            return NobelPrizeTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                NobelPrizeTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            NobelPrizeTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = NobelPrizeTestRunner.load_test_control()
...
def load_env_local():
    NobelPrizeTestRunner.load_env_local()


def env_override(m):
    return NobelPrizeTestRunner.env_override(m)


def entity_data(v):
    return NobelPrizeTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return NobelPrizeTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return NobelPrizeTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return NobelPrizeTestRunner.load_test_control()


def live_client_options():
    return NobelPrizeTestRunner.live_client_options()


def live_delay_ms():
    return NobelPrizeTestRunner.live_delay_ms()
