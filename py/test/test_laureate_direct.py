...
class TestLaureateDirect:

    def test_should_direct_list_laureate(self):
        setup = _laureate_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-laureate", "live" if setup["live"] else "unit")
        if _skip:
...
def _laureate_direct_setup(mockres):
    ...
    if live:
        ...
        client = NobelPrizeSDK(merged_opts)
        return {
            "client": client,
            "calls": calls,
            "live": True,
