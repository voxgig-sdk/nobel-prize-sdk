...
local LaureateEntity = {}
LaureateEntity.__index = LaureateEntity


function LaureateEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, LaureateEntity)
  self._name = "laureate"
  self._client = client
...
function LaureateEntity:get_name()
  return self._name
end


function LaureateEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return LaureateEntity.new(self._client, opts)
end
...
function LaureateEntity:mark_deleted()
  self._deleted = true
end


function LaureateEntity:deleted()
  return true == self._deleted
end


function LaureateEntity:data_set(args)
  if args ~= nil then
...
function LaureateEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function LaureateEntity:match_set(args)
  if args ~= nil then
...
function LaureateEntity:match_get()
...
function LaureateEntity:stream(action, args, callopts)
...
---@param reqmatch LaureateListMatch
---@param ctrl? table
---@return Laureate[]
---@return string? err
function LaureateEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function LaureateEntity:_run_op(ctx, post_done)
  local utility = self._utility
