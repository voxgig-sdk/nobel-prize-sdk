...
The Lua SDK for the NobelPrize API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:Laureate()` — each with the same small set of operations (`list`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
GitHub release tag (`lua/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/nobel-prize-sdk/releases)),
...
for _, item in ipairs(laureates) do
  print(item["id"], item["born"])
end
```
...
local result, err = client:Laureate():list()
...
Create an instance: `local laureate = client:Laureate(nil)`
...
Create an instance: `local prize = client:Prize(nil)`
...
```lua
local prizes, err = client:Prize():list()
```
...
```
lua/
├── nobel-prize_sdk.lua    -- Main SDK module
...
The main module (`nobel-prize_sdk`) exports the SDK constructor
...
```lua
local laureate = client:Laureate()
laureate:list()
