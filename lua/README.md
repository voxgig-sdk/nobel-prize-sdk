...
The Lua SDK for the NobelPrize API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Laureate(nil):list(nil, nil)
...
local result, err = client:NobelPrize(nil):load(
  { id = "test01" }, nil
)
