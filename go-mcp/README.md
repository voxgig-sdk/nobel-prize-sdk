...
[MCP](https://modelcontextprotocol.io) server exposing the NobelPrize SDK as
two agent tools — `nobel-prize_list` and `nobel-prize_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/nobel-prize-mcp)
make build

# 2. Provide credentials via the environment
export NOBEL_PRIZE_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user nobel-prize \
  -- /absolute/path/to/nobel-prize-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./nobel-prize-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// nobel-prize_list: first page of records
{ "entity": "laureate" }
{ "entity": "laureate", "query": { } }

// nobel-prize_load: one record by id
{ "entity": "laureate", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/nobel-prize-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user nobel-prize \
     -- "$PWD"/dist/*/nobel-prize-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `nobel-prize_list` and `nobel-prize_load` tools now appear
   in new sessions. Ask the agent to *"list laureate using nobel-prize"*
   and it calls `nobel-prize_list` with `{"entity":"laureate"}`.
...
```sh
export NOBEL_PRIZE_APIKEY=sk_live_xxx            # API key
export NOBEL_PRIZE_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `nobel-prize_list` tool
...
### Call the `nobel-prize_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `nobel-prize_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `nobel-prize_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `NOBEL_PRIZE_APIKEY` | API key sent with every request. |
| `NOBEL_PRIZE_BASE` | Optional override of the API base URL. |
...
```sh
./nobel-prize-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"nobel-prize_load","arguments":{"entity":"laureate","query":{"id":1}}}}'
```
