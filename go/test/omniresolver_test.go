...
import (
	...
	omni "github.com/voxgig-sdk/nobel-prize-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.NobelPrizeError); is {
				out := map[string]any{
					"name":    "NobelPrizeError",
...
func omniCtx(arg any, client *sdk.NobelPrizeSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.NobelPrizeSDK); is {
			client = live
