...
// argument it returns the current data as an Prize; with an argument it
// sets the data and returns the stored value. It delegates to the untyped Data
// (identical runtime) and converts at the typed boundary.
func (e *PrizeEntity) DataTyped(data ...Prize) Prize {
	if len(data) > 0 {
		return typedFrom[Prize](e.Data(asMap(data[0])))
	}
	return typedFrom[Prize](e.Data())
}

// MatchTyped mirrors DataTyped for the entity's match filter. The match is a
// partial of the entity, so it round-trips through Prize (all fields
// optional at the wire level).
func (e *PrizeEntity) MatchTyped(match ...Prize) Prize {
	if len(match) > 0 {
		return typedFrom[Prize](e.Match(asMap(match[0])))
	}
	return typedFrom[Prize](e.Match())
}
...
func (e *PrizeEntity) Stream(action string, args map[string]any, callopts map[string]any) <-chan any {
	out := make(chan any)
...
// PrizeListMatch and returns []Prize. It delegates to the untyped
// List (identical runtime) and converts at the typed boundary.
func (e *PrizeEntity) ListTyped(reqmatch PrizeListMatch, ctrl map[string]any) ([]Prize, error) {
	res, err := e.List(asMap(reqmatch), ctrl)
	if err != nil {
		return nil, err
	}
	return typedSliceFrom[Prize](res), nil
}
