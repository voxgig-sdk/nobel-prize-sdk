...
Provides information about Nobel Prizes and Laureates.

Learn more about Nobel Prize API at [nobelprize.org](https://nobelprize.org).
...
This SDK exposes the API as a small set of **semantic entities** — Laureate and Prize — that you
...
const items = await client.Laureate().list()
```
...
```go
client := sdk.Test()
result, err := client.Laureate(nil).List(
    nil, nil,
)
```
...
$laureates = $client->Laureate()->list();
print_r(array_map(fn($item) => $item->data_get(), $laureates));
```
