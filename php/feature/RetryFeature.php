...
class NobelPrizeRetryFeature extends NobelPrizeBaseFeature
{
    ...
    public function init(NobelPrizeContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (NobelPrizeContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(NobelPrizeContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(NobelPrizeContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
